 ...
 * @typedef {import('@octokit/webhooks').WebhookPayloadPullRequestPullRequest|import('@octokit/rest').Octokit.PullsListResponseItem} PullRequest
 * @typedef {import('@octokit/webhooks').WebhookPayloadPullRequestPullRequestBase|import('@octokit/rest').Octokit.PullsListResponseItemBase} PullRequestBase
...
async function getReviewApproval(context, pullRequest) {
  ...
  context.log.debug(`checking if ${PR(pullRequest)} is approved via reviews.`, config);
  ...
  if (effectiveReviews.some(isChangesRequested)) {
    context.log.debug(`skipping: ${PR(pullRequest)} reviews request changes`);

    return CHANGES_REQUESTED;
  }

  if (effectiveReviews.filter(isApproved).length < config.minApprovals) {
    context.log.debug(`skipping: ${PR(pullRequest)} lacks minApprovals=${config.minApprovals}`);
...
async function canMerge(context, pullRequest) {
  ...
  if (pullRequest.draft) {
    context.log(`skipping: ${PR(pullRequest)} is draft`);

    return false;
  }

  if ('merged' in pullRequest && pullRequest.merged) {
    context.log(`skipping: ${PR(pullRequest)} is already merged`);

    return false;
  }

  if ('rebaseable' in pullRequest && pullRequest.rebaseable === false) {
    context.log(`skipping: ${PR(pullRequest)} cannot be rebased`);
  ...
  context.log.debug(`check ${PR(pullRequest)} status and reviews`);
  ...
  if (statusApproval !== SUCCESS) {
    context.log(`skipping: ${PR(pullRequest)} failed status check (${statusApproval})`);
  ...
  if (reviewApproval !== APPROVED) {
    context.log(`skipping: ${PR(pullRequest)} failed review check (${reviewApproval})`);

    return false;
  }

  context.log.debug(`passed ${PR(pullRequest)} merge check`);
...
async function merge(context, pullRequest) {
  ...
  context.log.debug(`attempting to merge ${PR(pullRequest)}`);
  ...
  try {
    ...
    context.log(`merge ${PR(pullRequest)} failed (message=${error.message}, status=${error.status})`);
...
async function checkMerge(context, pullRequest) {
  ...
  context.log(`checking merge of ${PR(pullRequest)}`);

  const shouldMerge = await canMerge(context, pullRequest);

  if (!shouldMerge) {
    context.log(`skipping: merge of ${PR(pullRequest)} rejected`);
  ...
  if (merged) {
    ...
    context.log(`skipping: failed to merge ${PR(pullRequest)}`);
...
async function getTeamsApproval(context, pullRequest, config, reviews) {
  ...
  if (teams === null) {
    context.log(`skipping team approval for ${PR(pullRequest)}: could not fetch teams`);
  ...
  context.log.debug(`${PR(pullRequest)} effective review teams`, effectiveTeams);
...
async function getTeamsReviewApproval(context, pullRequest, reviews, teams, minApprovals) {
  ...
  for (const team of teams) {
    ...
    if (teamsApprovals.length < minApprovals) {
      context.log.debug(`skipping: ${PR(pullRequest)} lacks minApprovals=${minApprovals} by team ${team.name}`);
 ...
 * @param {Array<String>} reviewers list of reviewer logins
...
async function getTeamsWithMembers(context, pullRequest, reviewTeams) {
  ...
  if (org === null) {
    context.log(`skipping team review check on ${PR(pullRequest)}: pull request target not an organization`);
