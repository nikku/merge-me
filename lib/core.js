...
async function getReviewRequestsByTeams(config, context, pullRequest) {
  ...
  const { minApprovals, reviewTeams } = config;
  ...
  for (let i = 0; i < teams.length; i ++) {
    ...
    reponse = await GithubApi(context).teams.listMembers({
      org: orgName,
      team_id: id
    });

    const members = reponse.data;
  ...
  requested_reviewers.forEach(function(user) {
