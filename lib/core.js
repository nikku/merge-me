...
async function canMerge(context, pullRequest) {
  ...
  if (!pullRequest.rebaseable) {
    context.log.info(PR(pullRequest), 'skipping: pull request cannot be rebased');
  ...
  context.log.debug(PR(pullRequest), 'passed merge check');
...
async function merge(context, pullRequest) {
  ...
  context.log.debug(PR(pullRequest), 'attempting merge');
