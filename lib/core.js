...
async function findPullRequestByStatus(context, status) {
  ...
  // check if PR
  if (!branch) {
    context.log.info(ctx, 'skipping: no branch matches ref');
    return null;
  }

  context.log.debug(ctx, `checking branch ${branch.name}`);
  ...
  context.log.debug(ctx, `found ${pullRequests.length} pulls`);

  if (!pullRequests.length) {
    context.log.info(ctx, 'skipping: no PR matches ref');
    return null;
  }
...
function isStatusRelevantSuite(suite, pullRequest) {
  ...
  if (name === 'Dependabot' && status === 'queued') {
    return false;
  }
...
async function merge(context, pullRequest) {
  ...
  try {
    ...
    if ('status' in error) {
      ...
      context.log.error(ctx, `merge failed (message=${error.message})`);
...
async function getTeamsWithMembers(context, pullRequest, reviewTeams) {
  ...
  const teamMembers = await Promise.all(reviewTeams.map(async (teamSlug) => {
    ...
    const {
      ...
      if (error.status === 404) {
        throw MergeCheckError(`failed to fetch team ${org}/${teamSlug} (status=404)`);
      }
