 ...
 * @typedef { { minApprovals?: number, reviewTeams?: string[] } } AppConfig
 ...
 *
 * @typedef {import('probot').Context<import('@octokit/webhooks').EventPayloads.WebhookPayloadCheckSuite | import('@octokit/webhooks').EventPayloads.WebhookPayloadPullRequest | import('@octokit/webhooks').EventPayloads.WebhookPayloadPullRequestReview | import('@octokit/webhooks').EventPayloads.WebhookPayloadStatus>} ProbotContext
 */
...
async function findPullRequestByStatus(context, status) {
  ...
  // check if PR
  if (!branch) {
    context.log.info('skipping: no branch matches ref');
    return null;
  }
  ...
  const {
    ...
    head: `${repository.name}:${branch.name}`,
    state: 'open'
  }));
  ...
  if (!pullRequests.length) {
    context.log.info('skipping: no PR matches ref');
    return null;
  }
...
async function getReviewApproval(context, pullRequest) {
  ...
  context.log.debug(`found ${reviews.length} collaborator reviews`, PR(pullRequest));
...
async function getReviewConfig(context, pullRequest) {
  ...
  const {
    ...
    reviewTeams = []
  } = await context.config(CONFIG_FILE, /** @type {AppConfig} */ ({}));
...
async function canMerge(context, pullRequest) {
  ...
  if (pullRequest.draft) {
    context.log.info('skipping: pull request is a draft', PR(pullRequest));

    return false;
  }

  if (pullRequest.merged) {
    context.log.info('skipping: pull request is already merged', PR(pullRequest));
  ...
  if (!pullRequest.rebaseable) {
    context.log.info('skipping: pull request cannot be rebased', PR(pullRequest));
  ...
  if (statusApproval !== SUCCESS) {
    context.log.info(`skipping: failed status check (${statusApproval})`, PR(pullRequest));
  ...
  if (reviewApproval !== APPROVED) {
    context.log.info(`skipping: failed review check (${reviewApproval})`, PR(pullRequest));
...
async function merge(context, pullRequest) {
  ...
  try {
    ...
    if ('status' in error) {
      ...
      context.log.info(`merge failed (message=${error.message}, status=${error.status})`, PR(pullRequest));
...
async function checkMerge(context, pullRequest) {
  ...
  context.log.info('checking merge', PR(pullRequest));
  ...
  if (!shouldMerge) {
    context.log.info('skipping: merge rejected', PR(pullRequest));
  ...
  if (merged) {
    context.log.info('merged', PR(pullRequest));

    return true;
  } else {
    context.log.info('skipping: merge failed', PR(pullRequest));
