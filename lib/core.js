...
async function getReviewApproval(context, pullRequest) {
  ...
  if (reviewTeams.length) {
    ...
    const teamsApproval = await getTeamsApproval(context, pullRequest, config, effectiveReviews, allApproved);
...
async function getTeamsApproval(context, pullRequest, config, allReviews, approvedReviews) {
  ...
  const teamsReviewContext = await getTeamsReviewContext(context, pullRequest, reviewTeams, allReviews);

  if (!teamsReviewContext) {
    context.log.debug(`no team review context for #${number}, ignoring`);
    return null;
  }

  const {
    effectiveReviewTeams,
    teamByUserName
  } = teamsReviewContext;

  context.log.debug(`#${number} requires approvals by teams`, effectiveReviewTeams);

  return getTeamReviewApproval(approvedReviews, effectiveReviewTeams, teamByUserName, minApprovals);
}


async function getTeamsReviewContext(context, pullRequest, reviewTeams, allReviews) {

  const orgName = getOrgName(context, pullRequest);

  if (orgName === null) {
    context.log('skipping team review check on #{pullRequest.number}: target not an organization');
  ...
  const {
    data: allTeams
  } = await GithubApi(context).teams.list({ org: orgName });

  // filter out unconfigured teams
  const teams = allTeams.filter(function(team) {
    return reviewTeams.includes(team.name);
  });
  ...
  for (let i = 0; i < teams.length; i ++) {
    ...
    // fill teamByUserName map
    members.forEach(function(member) {

      const { login } = member;

      const existingTeamName = teamByUserName[login];
  ...
  allReviewers.forEach(function(user) {

    const { login } = user;
    const userTeam = teamByUserName[login];
  ...
  return {
    effectiveReviewTeams: Object.keys(reviewRequestsByTeams),
    teamByUserName
  };
}

function getTeamReviewApproval(allApproved, reviewTeams, teamByUserName, minApprovals) {
  ...
  allApproved.forEach(function(approval) {
    ...
    const team = teamByUserName[userName];
