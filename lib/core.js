...
async function getTeamsApproval(context, pullRequest, config, allReviews, approvedReviews) {
  ...
  // TODO(nikku): intoduce error handling for this edge case
  if (!teamsReviewContext) {
    return APPROVED;
  }

  const {
    reviewRequestsByTeams,
    teamsByUserName
  } = teamsReviewContext;

  context.log.debug(`#${number} requires approvals by teams`, Object.keys(reviewRequestsByTeams));

  return getTeamReviewApproval(approvedReviews, Object.keys(reviewRequestsByTeams), teamsByUserName, minApprovals);
}
